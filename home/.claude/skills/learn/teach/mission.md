...
`MISSION.md` sits at the root of the private lane and captures the *reason* the user is learning
...
A wrong mission is worse than no mission, because it steers every session after it.
...
## Success looks like
- {A specific, observable thing they will be able to do}
- {Another}
...
## Out of scope
- {Adjacent topics they are deliberately not chasing, which protects the zone of proximal
  development}
```

## Rules

- **One mission per topic.** Two unrelated things are two workspaces under `private/teach/`.
