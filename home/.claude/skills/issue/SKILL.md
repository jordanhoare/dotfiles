...
If you have not already explored the codebase, do so to understand the current state of the code. Issue titles and descriptions should use the project's domain glossary at `docs/internal/glossary.md`, and respect ADRs in the area you're touching.
...
Publish the approved issues. **How** depends on the tracker `/setup` configured; the issues are the same either way, only the shape of the blocking edges changes:
