...
- [oos.md](oos.md): how the `.out-of-scope/` knowledge base works
...
4. **Grill (if needed).** If the request needs fleshing out, call the Skill tool twice, for "grilling" and "domain", and grill it into shape a round of questions at a time, sharpening domain terms and updating `docs/internal/glossary.md`/ADRs inline as decisions land.
...
5. **Apply the outcome:**
   ...
   - For `wontfix`, close the issue, with the comment depending on *why*:
     ...
     - **Rejected (enhancement)**: write to `.out-of-scope/`, link to it from a comment, then close ([oos.md](oos.md)).
